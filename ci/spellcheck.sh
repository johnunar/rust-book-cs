...
if [[ ! -f "$dict_filename" ]]; then
    ...
    echo "personal_ws-1.1 cs 0 utf-8" > "$dict_filename"
    cat "${markdown_sources[@]}" | aspell --ignore 3 -l cs list | sort -u >> "$dict_filename"
elif [[ "$mode" == "list" ]]; then
    ...
    for fname in "${markdown_sources[@]}"; do
        command=$(aspell --ignore 3 -l cs --personal="$dict_path" "$mode" < "$fname")
        if [[ -n "$command" ]]; then
    ...
    for fname in "${markdown_sources[@]}"; do
        aspell --ignore 3 -l cs --dont-backup --personal="$dict_path" "$mode" "$fname"
    done
fi
